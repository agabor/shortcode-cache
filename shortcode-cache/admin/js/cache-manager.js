(function ($) {
    $(document).ready(function () {
        ...
        $(document).on('click', '.shortcode-cache-clear-role-group-btn', function (e) {
            e.preventDefault();
            e.stopPropagation();

            const button = $(this);

            if ( ! confirm( 'Are you sure you want to clear all cached items in this role group? This action cannot be undone.' ) ) {
                return;
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        button.prop('disabled', false);
                        button.text('Clear Role Group Cache');
                        alert(response.data.message);
                    }
                },
                error: function () {
                    button.prop('disabled', false);
                    button.text('Clear Role Group Cache');
                    alert('An error occurred while clearing the role group cache.');
                },
            });
        });
